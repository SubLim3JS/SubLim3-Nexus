...
async function deleteCard(card) { if (!await nexusConfirm(`Delete the binding for ${card.name}?`, { okLabel:"Delete" })) return; try { await api(`/api/v1/rfid/cards/${encodeURIComponent(card.uid)}`, { method:"DELETE" }); await reloadCards(); pageMessage(`${card.name} deleted.`, "success"); } catch (error) { pageMessage(error.message, "error"); } }
