...
$("#gm-pair-form").addEventListener("submit",async(event)=>{event.preventDefault();const message=$("#gm-pair-message");try{const body=await api("/api/v1/auth/pair",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({role:"gm",pin:$("#gm-pin").value,campaign_id:$("#gm-campaign").value})},false);gmToken=body.token;localStorage.setItem("nexus-gm-token",gmToken);await openConsole();}catch(error){message.textContent=error.message;message.className="form-message error";}});
