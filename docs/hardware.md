...
Nexus Core supports an RC522 RFID reader and two active-low media buttons on a
Raspberry Pi 4 or 5. The installer enables SPI, installs `python3-spidev` and
`python3-gpiozero`, and gives the `nexus` service account GPIO/SPI access.
...
The driver reads 4-, 7-, and 10-byte UIDs and publishes card placement and
release events through the existing RFID service.
...
A hold is recognized after 700 ms. Two taps within 350 ms toggle playback.
Single-tap volume changes use the step and maximum configured on the Settings
page. Previous and next wrap through the managed ambience library.
