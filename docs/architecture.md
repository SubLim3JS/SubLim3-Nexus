...
The Settings page at `/settings/` uses a six-digit installer-generated PIN for connectivity mutations. Nexus Core remains unprivileged; a root-owned helper exposes only validated NetworkManager and BlueZ actions. Failed home Wi-Fi connections restore the Nexus hotspot, and a boot recovery service retries the saved Home Wi-Fi connection before starting Local Mode whenever no Wi-Fi connection is available. Local Mode uses `NEXUS_HOTSPOT_ADDRESS`, which defaults to `10.99.0.1/24`. The retry window is controlled by `NEXUS_HOME_RECONNECT_ATTEMPTS` and `NEXUS_HOME_RECONNECT_DELAY_SECONDS`.
