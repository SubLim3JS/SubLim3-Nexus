...
before(async () => {
  ...
  server = createServer(createApp({
    ...
    settingsPin: "123456",
    connectivity: {
      status: async () => ({ supported: true, wifi: { mode: "local", ssid: "SubLim3-Nexus", addresses: ["10.10.10.1/24"] }, bluetooth: { available: true, visible: false, connected_devices: [] } }),
...
test("serves Android app release metadata", async () => {
  ...
  assert.equal(catalog.apps.owner.versionName, "0.1.13");
  assert.equal(catalog.apps.owner.versionCode, 14);
  ...
  assert.equal(catalog.apps.player.versionName, "0.1.8");
  assert.equal(catalog.apps.player.versionCode, 9);
