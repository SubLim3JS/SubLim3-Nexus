...
before(async () => {
  ...
  const access = new AccessService({ sessionStore: accessSessionStore, adminPin: "111111", gmPin: "222222" });
...
test("scopes GM access to one campaign and permits table mutations", async () => {
  const gm = await pair({ role: "gm", pin: "222222", campaign_id: "green_realm" });
